...
from sklearn.feature_selection import VarianceThreshold,SelectPercentile,chi2,RFECV,SelectFromModel
...
class ClassificationApp(BaseApp):
    ...
    def _init_parser(self):
        ...
        self.parser.add_argument("dataset", type=str,
                                 help="SVM light format dataset. If \'toy\' is given then it is used 20ng as a toy example.",
                                 default='toy')

        self.parser.add_argument("-m", "--method", choices=models, default=list(models)[0])

        self.parser.add_argument("-j", "--n_jobs", type=int,
                                 help='Number of CPUs available to parallelize the execution (Default:1). If -1 is given then it gets all CPUs available',
                                 default=1)

        self.parser.add_argument("--trials", type=int,
                                 help='Number of trials (Default:10).', default=10)

        self.parser.add_argument("--cv", type=int,
                                 help='Search for best parameters using grid search cross-validation (Default:1). When 1 is given there will not be search at all.',
                                 default=1)

        self.parser.add_argument("--start_fold", type=int, help='Starting trial (default: 1).',
                                 default=1)

        self.parser.add_argument("-s", "--seed", type=int,
                                 help='Random seed initializing the pseudo-random number generator (default: 42).',
                                 default=42)

        self.parser.add_argument("--test", action='store_true',
                                 help='Executes only one trial. It is used when you want to make a rapid test.')

        self.parser.add_argument("-o", "--output", type=str, help="Output file for results.", default="")

        self.parser.add_argument("-n", "--norm", help='Dataset sample normalization', choices=["max", "l1", "l2"],
                                 default=None)
        ...
        for train_index, test_index in skf.split(X, y):
            ...
            if args.dump_meta_level != "":
                if hasattr(e, 'oob_decision_function_'):
                    ...
                    dump_svmlight_file(mt.fit_transform(X_train, y_train), y_train,
                                       args.dump_meta_level % ("train", args.method, k))
                    dump_svmlight_file(e.predict_proba(X_test), y_test, args.dump_meta_level % ("test", args.method, k))
...
class TextClassificationApp(ClassificationApp):
    ...
    def _init_parser(self):
        ...
        self.parser.add_argument("-g", "--n_gpus", type=int,
                                 help='Number of GPUs available to execute kNN-based classifiers (Default:1).',
                                 default=1)

        self.parser.add_argument("-i", "--n_iterations", type=int,
                                 help='Number of iteration for broof to perform (Default:200).', default=200)

        self.parser.add_argument("-t", "--trees", type=int, help='Number of trees (Default:200).', default=200)

        self.parser.add_argument("-k", "--n_neighbors", type=int, help='Number of nearest neirghbors (Default:30).',
                                 default=30)

        self.parser.add_argument("-f", "--max_features",
                                 help='The number of features to consider when looking for the best split (Default:sqrt).',
                                 default='sqrt')
        ...
        self.parser.add_argument("--learning_rate", type=float,
                                 help='Algorithm learning rate. It controls algorithm\'s convergence.', default=1.0)

        self.parser.add_argument("-a", "--alpha", type=float,
                                 help='Additive (Laplace/Lidstone) smoothing parameter (0 for no smoothing).(Default: 1)',
                                 default=1)

        self.parser.add_argument("-c", "--C", type=float,
                                 help='Penalty parameter C of the error term. For SVM training. (Default: 1)',
                                 default=1)
...
class StackerApp(TextClassificationApp):
    ...
    def _create_stacking(self, args):
        ...
        base_level = [Pipeline(
            default_transformers[estimator] + [('estimator', self.instantiator.get_instance(estimator, params[i]))]) for
                      i, estimator in enumerate(base_classifiers)]
        ...
        meta_level = [Pipeline(
            default_transformers[estimator] + [('estimator', self.instantiator.get_instance(estimator, params[i]))]) for
                      i, estimator in enumerate(meta_classifiers)]
